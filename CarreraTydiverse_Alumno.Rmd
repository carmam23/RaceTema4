...
#Podemos observar que al aplicar la función glimpse sobre cran, podemos visualizar los datos de forma horizontal, en lugar de en vertical que es como observamos los datos por defecto al poner únicamente el nombre de la variable. También la función glimpse nos da el número de filas y columnas
...
```{r}
#Escogemos las variable haciendo uso de '$'
cran$ip_id
cran$package
cran$country
```
...
```{r}
#Escogemos las variables utilizando select
...
```{r}
#Utilizamos el select con el '-' para no obtener las columnas entre date y size
...
```{r}
#Elegimos los registros usando filter, y vemos que solo hay un registro que cumple las cracterísticas
...
```{r}
summarise(by_package, median(size))
```
...
umbral2 <- quantile(pack_sum$count, probs = 0.99)
